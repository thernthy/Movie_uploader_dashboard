...
import UploadEncodingZip from "./drag";
...
const EncodingUpladerIndex: React.FC = () => {
  ...
  const HandleDeleteVideos = async () => {
    if (selectedData.length > 0) {
      try {
        ...
        setSelectedData([]);
        setSelectedCollection("");
        handleRefresh(); // This function should be updated if needed based on new requirements
      } catch (error) {
    ...
    <>
      <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
        <div className="w-full rounded-md bg-white dark:bg-meta-4">
          ...
          >
            <SwiperSlide>
              <div className="grid items-center gap-2 px-2 py-2 lg:grid-cols-3 xl:grid-cols-3">
        ...
        <div>
          <UploadEncodingZip handleRefresh={handleRefresh} />
        </div>
      </div>
...
export default EncodingUpladerIndex;
