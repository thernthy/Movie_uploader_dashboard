...
interface SearchPageProps {
  ...
  current_page?: Number;
  pageCount?: Number;
  handlePagination: (event: React.ChangeEvent<unknown>, value: number) => void; // Define as a function
}
...
const SearchPage: React.FC<SearchPageProps> = ({
  ...
  const [pagCount, setPageCount] = useState(pageCount || 0);
  ...
  return (
    <>
      <div
        className="grid max-h-screen grid-cols-1 gap-2 md:grid-cols-1"
        style={{ height: "85vh" }}
      >
        <div className="w-full rounded-md bg-white dark:bg-meta-4">
          <div className="mb-2 flex w-full items-center justify-between">
            <div className="inline-flex items-center rounded-md bg-white p-1.5 dark:bg-meta-4">
              <button className=" rounded-l bg-white px-3 py-3 text-xs font-medium text-black shadow-card hover:bg-white hover:shadow-card dark:bg-boxdark dark:text-white dark:hover:bg-boxdark">
                Result for keyword: {query}{" "}
                <span
                  className={`${
                    results.length > 0 ? "text-green-400" : "text-red-600"
                  }`}
                >
              ...
              <button className=" rounded-r bg-white px-3 py-1 text-xs font-medium text-black shadow-card hover:bg-white hover:shadow-card dark:bg-boxdark dark:text-white dark:hover:bg-boxdark">
                <Pagination
